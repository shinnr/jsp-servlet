...
<c:url var="zipcodeSearchURI" value="/09/zipcodeSearchForm.jsp"></c:url>
...
<!-- 
	javascript 유니코드 (UTF-8) 인코딩			/ 		디코딩
							escape()			unescape() : 유니코드 형식으로 인코딩
															URL 표현시 활용 기호 (: or / or ? etc..)을 포함
							encodeURI()			decodeURI() : 유니코드 형식으로 인코딩
															URL 표현시 활용 기호 (: or / or ? etc..)을 미포함
					encodeURIComponent()		decodeURIComponent() : 유니코드 형식으로 인코딩
																		단순 문자 또는 문자열
...
function zipCodePopup(){
	//document 팝업 - 모달 : 출력되는 팝업에 존재하는 포커스를 잃지않으면(팝업종료), 포커스가 이동이 불가.
	//				모달리스 : 출력되는 팝업에 포커스가 존재해도 자유롭게 포커스가 이동이 가능.
	//아귀먼트 : 1. url
	//		  2. 팝업의 타이틀
	//		  3. 옵션 (브라우저별 상이)
	var url = '${zipcodeSearchURI}';
